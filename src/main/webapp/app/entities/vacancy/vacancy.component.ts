...
export class VacancyComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.vacancyService
      ...
      .subscribe(
        (res: HttpResponse<IVacancy[]>) => this.paginateVacancies(res.body, res.headers),
  ...
  trackId(index: number, item: IVacancy) {
    return item.id;
  }
  ...
  protected paginateVacancies(data: IVacancy[], headers: HttpHeaders) {
